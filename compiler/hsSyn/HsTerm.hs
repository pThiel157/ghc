...
-- | Abstract Haskell syntax for terms.
module HsTerm where
...
mkTwoFS :: (FastString, FastString) -> GenData
...
-- for getChars, getPRIMCHARs, getPRIMEINTEGERs, getPRIMSTRINGs, getPRIMWORDs
mkOneSRT :: SourceText -> GenData
mkOneSRT srt = OneSRT srt

mkOneInt :: Integer -> GenData
mkOneInt int = OneInt Integer

-- for getPRIMESTRING
mkOneBString :: ByteString -> GenData
mkOneBString bs = OneBString bs

-- for getPRIMFLOAT and getPRIMDOUBLE
mkOneFracLit :: FractionalLit -> GenData
mkOneFracLit fl = OneFracLit fl
-}
--EF
...
-------------------------
-- | Post-Type checking Term
--
-- PostTcTerm is an evidence term attached to the syntax tree by the
-- type checker (c.f. postTcType).
type PostTcTerm  = HsTerm GhcTc

-- | Post-Type checking Table
--
-- We use a PostTcTable where there are a bunch of pieces of evidence, more
-- than is convenient to keep individually.
type PostTcTable = [(Name, PostTcTerm)]

-------------------------
-- | Syntax Term
--
-- SyntaxTerm is like 'PostTcTerm', but it's filled in a little earlier,
...
--
-- > syn_res_wrap $ syn_term (syn_arg_wraps[0] arg0)
-- >                         (syn_arg_wraps[1] arg1) ...
--
-- where the actual arguments come from elsewhere in the AST.
-- This could be defined using @GhcPass p@ and such, but it's
-- harder to get it all to work out that way. ('noSyntaxTerm' is hard to
-- write, for example.)
data SyntaxTerm p = SyntaxTerm { syn_term      :: HsTerm p
                               , syn_arg_wraps :: [HsWrapper]
                               , syn_res_wrap  :: HsWrapper }

-- | This is used for rebindable-syntax pieces that are too polymorphic
-- for tcSyntaxOp (trS_fmap and the mzip in ParStmt)
noTerm :: HsTerm (GhcPass p)
noTerm = HsLit noExt (HsString (SourceText  "noTerm") (fsLit "noTerm"))

noSyntaxTerm :: SyntaxTerm (GhcPass p)
                              -- Before renaming, and sometimes after,
                              -- (if the syntax slot makes no sense)
noSyntaxTerm = SyntaxTerm { syn_term      = HsLit noExt (HsString NoSourceText
                                                        (fsLit "noSyntaxTerm"))
                          , syn_arg_wraps = []
                          , syn_res_wrap  = WpHole }

-- | Make a 'SyntaxTerm Name' (the "rn" is because this is used in the
-- renamer), missing its HsWrappers.
mkRnSyntaxTerm :: Name -> SyntaxTerm GhcRn
mkRnSyntaxTerm name = SyntaxTerm { syn_term      = HsVar noExt $ noLoc name
                                 , syn_arg_wraps = []
                                 , syn_res_wrap  = WpHole }
...
instance (p ~ GhcPass pass, OutputableBndrId p)
       => Outputable (SyntaxTerm p) where
  ppr (SyntaxTerm { syn_term      = term
                  , syn_arg_wraps = arg_wraps
                  , syn_res_wrap  = res_wrap })
    = sdocWithDynFlags $ \ dflags ->
      getPprStyle $ \s ->
      if debugStyle s || gopt Opt_PrintExplicitCoercions dflags
      then ppr term <> braces (pprWithCommas ppr arg_wraps)
                    <> braces (ppr res_wrap)
      else ppr term

-- | Command Syntax Table (for Arrow syntax)
type CmdSyntaxTable p = [(Name, HsTerm p)]
...
* After the type checker, it takes the form [(std_name, <term>)]
  where <term> is the evidence for the method.  This evidence is
...
is Less Cool because
  * The renamer has to predict which methods are needed.
    See the tedious RnTerm.methodNamesCmd.
...
data UnboundVar
  ...
  | TrueTermHole OccName             -- ^ A "true" term hole (_ or _x)

  deriving Data

instance Outputable UnboundVar where
    ppr (OutOfScope occ _) = text "OutOfScope" <> parens (ppr occ)
    ppr (TrueTermHole occ) = text "TermHole"   <> parens (ppr occ)

unboundVarOcc :: UnboundVar -> OccName
unboundVarOcc (OutOfScope occ _) = occ
unboundVarOcc (TrueTermHole occ) = occ
...
data RecordConTc = RecordConTc
      ...
      , rcon_con_term :: PostTcTerm   -- Instantiated constructor function
      }
...
type instance XTermWithTySig GhcPs = (LHsSigWcType GhcPs)
type instance XTermWithTySig GhcRn = (LHsSigWcType GhcRn)
type instance XTermWithTySig GhcTc = (LHsSigWcType GhcRn)

type instance XArithSeq      GhcPs = NoExt
type instance XArithSeq      GhcRn = NoExt
type instance XArithSeq      GhcTc = PostTcTerm
...
data HsTupArg id
  = Present (XPresent id) (LHsTerm id)     -- ^ The argument
...
the RHS so that we can build the term.
...
instance (p ~ GhcPass pass, OutputableBndrId p) => Outputable (HsTerm p) where
    ppr term = pprTerm term

-----------------------
-- pprTerm, pprLTerm, pprBinds call pprDeeper;
-- the underscore versions do not
pprLTerm :: (OutputableBndrId (GhcPass p)) => LHsTerm (GhcPass p) -> SDoc
pprLTerm (L _ e) = pprTerm e

pprTerm :: (OutputableBndrId (GhcPass p)) => HsTerm (GhcPass p) -> SDoc
pprTerm e | isAtomicHsTerm e || isQuietHsTerm e =            ppr_term e
          | otherwise                           = pprDeeper (ppr_term e)

isQuietHsTerm :: HsTerm id -> Bool
-- Parentheses do display something, but it gives little info and
-- if we go deeper when we go inside them then we get ugly things
-- like (...)
isQuietHsTerm (HsPar {})        = True
-- applications don't display anything themselves
isQuietHsTerm (HsApp {})        = True
isQuietHsTerm (HsAppType {})    = True
isQuietHsTerm (OpApp {})        = True
isQuietHsTerm _ = False
...
-----------------------
ppr_lterm :: (OutputableBndrId (GhcPass p)) => LHsTerm (GhcPass p) -> SDoc
ppr_lterm e = ppr_term (unLoc e)

ppr_term :: forall p. (OutputableBndrId (GhcPass p))
         => HsTerm (GhcPass p) -> SDoc
ppr_term (HsVar _ (L _ v))  = pprPrefixOcc v
ppr_term (HsUnboundVar _ uv)= pprPrefixOcc (unboundVarOcc uv)
ppr_term (HsConLikeOut _ c) = pprPrefixOcc c
ppr_term (HsIPVar _ v)      = ppr v
ppr_term (HsOverLabel _ _ l)= char '#' <> ppr l
ppr_term (HsLit _ lit)      = ppr lit
ppr_term (HsOverLit _ lit)  = ppr lit
ppr_term (HsPar _ e)        = parens (ppr_lterm e)

ppr_term (HsCoreAnn _ stc (StringLiteral sta s) e)
  = vcat [pprWithSourceText stc (text "{-# CORE")
          <+> pprWithSourceText sta (doubleQuotes $ ftext s) <+> text "#-}"
         , ppr_lterm e]

ppr_term e@(HsApp {})        = ppr_apps e []
ppr_term e@(HsAppType {})    = ppr_apps e []

ppr_term (OpApp _ e1 op e2)
  ...
  where
    ...
    should_print_infix (HsUnboundVar _ h@TrueTermHole{})
    ...
    pp_e1 = pprDebugParendTerm opPrec e1   -- In debug mode, add parens
    pp_e2 = pprDebugParendTerm opPrec e2   -- to make precedence clear
...
ppr_term (NegApp _ e _) = char '-' <+> pprDebugParendTerm appPrec e

ppr_term (SectionL _ term op)
  = case unLoc op of
      HsVar _ (L _ v)  -> pp_infixly v
      HsConLikeOut _ c -> pp_infixly (conLikeName c)
      HsUnboundVar _ h@TrueTermHole{}
                       -> pp_infixly (unboundVarOcc h)
      _                -> pp_prefixly
  where
    pp_term = pprDebugParendTerm opPrec term

    pp_prefixly = hang (hsep [text " \\ x_ ->", ppr op])
                       4 (hsep [pp_term, text "x_ )"])

    pp_infixly :: forall a. (OutputableBndr a) => a -> SDoc
    pp_infixly v = (sep [pp_term, pprInfixOcc v])

ppr_term (SectionR _ op term)
  = case unLoc op of
      HsVar _ (L _ v)  -> pp_infixly v
      HsConLikeOut _ c -> pp_infixly (conLikeName c)
      HsUnboundVar _ h@TrueTermHole{}
  ...
  where
    ...
    pp_infixly :: forall a. (OutputableBndr a) => a -> SDoc
    pp_infixly v = sep [pprInfixOcc v, pp_term]

ppr_term (ExplicitTuple _ terms boxity)
  = tupleParens (boxityTupleSort boxity) (fcat (ppr_tup_args $ map unLoc terms))
  where
    ppr_tup_args []               = []
    ppr_tup_args (Present _ e : es) = (ppr_lterm e <> punc es) : ppr_tup_args es
...
ppr_term (ExplicitSum _ alt arity term)
  = text "(#" <+> ppr_bars (alt - 1) <+> ppr term <+> ppr_bars (arity - alt) <+> text "#)"
  where
    ppr_bars n = hsep (replicate n (char '|'))

ppr_term (HsLam _ matches)
  = pprMatches matches

ppr_term (HsLamCase _ matches)
  = sep [ sep [text "\\case"],
          nest 2 (pprMatches matches) ]

ppr_term (HsCase _ term matches@(MG { mg_alts = L _ [_] }))
  = sep [ sep [text "case", nest 4 (ppr term), ptext (sLit "of {")],
          nest 2 (pprMatches matches) <+> char '}']
ppr_term (HsCase _ term matches)
  = sep [ sep [text "case", nest 4 (ppr term), ptext (sLit "of")],
          nest 2 (pprMatches matches) ]

ppr_term (HsIf _ _ e1 e2 e3)
  = sep [hsep [text "if", nest 2 (ppr e1), ptext (sLit "then")],
         nest 4 (ppr e2),
         text "else",
         nest 4 (ppr e3)]

ppr_term (HsMultiIf _ alts)
  = hang (text "if") 3  (vcat (map ppr_alt alts))
  where ppr_alt (L _ (GRHS _ guards term)) =
          hang vbar 2 (ppr_one one_alt)
          where
            ppr_one [] = panic "ppr_exp HsMultiIf"
            ppr_one (h:t) = hang h 2 (sep t)
            one_alt = [ interpp'SP guards
                      , text "->" <+> pprDeeper (ppr term) ]
        ppr_alt (L _ (XGRHS x)) = ppr x

-- special case: let ... in let ...
ppr_term (HsLet _ (L _ binds) term@(L _ (HsLet _ _ _)))
  = sep [hang (text "let") 2 (hsep [pprBinds binds, ptext (sLit "in")]),
         ppr_lterm term]

ppr_term (HsLet _ (L _ binds) term)
  = sep [hang (text "let") 2 (pprBinds binds),
         hang (text "in")  2 (ppr term)]

ppr_term (HsDo _ do_or_list_comp (L _ stmts)) = pprDo do_or_list_comp stmts

ppr_term (ExplicitList _ _ terms)
  = brackets (pprDeeperList fsep (punctuate comma (map ppr_lterm terms)))

ppr_term (RecordCon { rcon_con_name = con_id, rcon_flds = rbinds })
  = hang (ppr con_id) 2 (ppr rbinds)

ppr_term (RecordUpd { rupd_term = L _ aexp, rupd_flds = rbinds })
  = hang (ppr aexp) 2 (braces (fsep (punctuate comma (map ppr rbinds))))

ppr_term (TermWithTySig sig term)
  = hang (nest 2 (ppr_lterm term) <+> dcolon)
         4 (ppr sig)

ppr_term (ArithSeq _ _ info) = brackets (ppr info)

ppr_term (EWildPat _)     = char '_'
ppr_term (ELazyPat _ e)   = char '~' <> ppr e
ppr_term (EAsPat _ v e)   = ppr v <> char '@' <> ppr e
ppr_term (EViewPat _ p e) = ppr p <+> text "->" <+> ppr e

ppr_term (HsSCC _ st (StringLiteral stl lbl) term)
...
ppr_term (HsWrap _ co_fn e)
  = pprHsWrapper co_fn (\parens -> if parens then pprTerm e
                                             else pprTerm e)

ppr_term (HsSpliceE _ s)         = pprSplice s
ppr_term (HsBracket _ b)         = pprHsBracket b
ppr_term (HsRnBracketOut _ e []) = ppr e
ppr_term (HsRnBracketOut _ e ps) = ppr e $$ text "pending(rn)" <+> ppr ps
ppr_term (HsTcBracketOut _ e []) = ppr e
ppr_term (HsTcBracketOut _ e ps) = ppr e $$ text "pending(tc)" <+> ppr ps

ppr_term (HsProc _ pat (L _ (HsCmdTop _ cmd)))
  = hsep [text "proc", ppr pat, ptext (sLit "->"), ppr cmd]
ppr_term (HsProc _ pat (L _ (XCmdTop x)))
  = hsep [text "proc", ppr pat, ptext (sLit "->"), ppr x]

ppr_term (HsStatic _ e)
  = hsep [text "static", ppr e]

ppr_term (HsTick _ tickish exp)
  = pprTicks (ppr exp) $
    ppr tickish <+> ppr_lterm exp
ppr_term (HsBinTick _ tickIdTrue tickIdFalse exp)
  = pprTicks (ppr exp) $
    hcat [text "bintick<",
          ppr tickIdTrue,
          text ",",
          ...
          text ">(",
          ppr exp, text ")"]
ppr_term (HsTickPragma _ _ externalSrcLoc _ exp)
  = pprTicks (ppr exp) $
...
ppr_term (HsArrApp _ arrow arg HsFirstOrderApp True)
  = hsep [ppr_lterm arrow, larrowt, ppr_lterm arg]
ppr_term (HsArrApp _ arrow arg HsFirstOrderApp False)
  = hsep [ppr_lterm arg, arrowt, ppr_lterm arrow]
ppr_term (HsArrApp _ arrow arg HsHigherOrderApp True)
  = hsep [ppr_lterm arrow, larrowtt, ppr_lterm arg]
ppr_term (HsArrApp _ arrow arg HsHigherOrderApp False)
  = hsep [ppr_lterm arg, arrowtt, ppr_lterm arrow]

ppr_term (HsArrForm _ (L _ (HsVar _ (L _ v))) (Just _) [arg1, arg2])
  = sep [pprCmdArg (unLoc arg1), hsep [pprInfixOcc v, pprCmdArg (unLoc arg2)]]
ppr_term (HsArrForm _ (L _ (HsConLikeOut _ c)) (Just _) [arg1, arg2])
  = sep [pprCmdArg (unLoc arg1), hsep [pprInfixOcc (conLikeName c), pprCmdArg (unLoc arg2)]]
ppr_term (HsArrForm _ op _ args)
  = hang (text "(|" <+> ppr_lterm op)
...
ppr_apps :: (OutputableBndrId (GhcPass p))
         => HsTerm (GhcPass p)
         -> [Either (LHsTerm (GhcPass p)) (XAppTypeE (GhcPass p))]
         -> SDoc
ppr_apps (HsApp _ (L _ fun) arg)        args
  = ppr_apps fun (Left arg : args)
ppr_apps (HsAppType arg (L _ fun))    args
  = ppr_apps fun (Right arg : args)
ppr_apps fun args = hang (ppr_term fun) 2 (sep (map pp args))
  where
    pp (Left arg)                             = ppr arg
...
pprParendLTerm (but don't print double parens of course).
...
pprDebugParendTerm :: (OutputableBndrId (GhcPass p))
                   => PprPrec -> LHsTerm (GhcPass p) -> SDoc
pprDebugParendTerm p term
  = getPprStyle (\sty ->
    if debugStyle sty then pprParendLTerm p term
                      else pprLTerm      term)

pprParendLTerm :: (OutputableBndrId (GhcPass p))
               => PprPrec -> LHsTerm (GhcPass p) -> SDoc
pprParendLTerm p (L _ e) = pprParendTerm p e

pprParendTerm :: (OutputableBndrId (GhcPass p))
              => PprPrec -> HsTerm (GhcPass p) -> SDoc
pprParendTerm p term
  | hsTermNeedsParens p term = parens (pprTerm term)
  | otherwise                = pprTerm term
        -- Using pprLTerm makes sure that we go 'deeper'
        -- I think that is usually (always?) right

-- | @'hsTermNeedsParens' p e@ returns 'True' if the term @e@ needs
-- parentheses under precedence @p@.
hsTermNeedsParens :: PprPrec -> HsTerm p -> Bool
hsTermNeedsParens p = go
  where
    go (HsVar{})                      = False
    ...
    go (HsLet{})                      = p > topPrec
    go (HsDo _ sc _)
      | isListCompTerm sc             = False
      | otherwise                     = p > topPrec
    go (ExplicitList{})               = False
    go (RecordUpd{})                  = False
    go (TermWithTySig{})              = p > topPrec
    go (ArithSeq{})                   = False
    go (EWildPat{})                   = False
    go (ELazyPat{})                   = False
    go (EAsPat{})                     = False
...
parenthesizeHsTerm :: PprPrec -> LHsTerm (GhcPass p) -> LHsTerm (GhcPass p)
parenthesizeHsTerm p le@(L loc e)
  | hsTermNeedsParens p e = L loc (HsPar NoExt le)
  | otherwise             = le

isAtomicHsTerm :: HsTerm id -> Bool
-- True of a single token
isAtomicHsTerm (HsVar {})        = True
isAtomicHsTerm (HsConLikeOut {}) = True
isAtomicHsTerm (HsLit {})        = True
isAtomicHsTerm (HsOverLit {})    = True
isAtomicHsTerm (HsIPVar {})      = True
isAtomicHsTerm (HsOverLabel {})  = True
isAtomicHsTerm (HsUnboundVar {}) = True
isAtomicHsTerm (HsWrap _ _ e)    = isAtomicHsTerm e
isAtomicHsTerm (HsPar _ e)       = isAtomicHsTerm (unLoc e)
isAtomicHsTerm (HsRecFld{})      = True
isAtomicHsTerm _                 = False
...
We re-use HsTerm to represent these.
-}
...
data HsCmd id
  ...
  = HsCmdArrApp          -- Arrow tail, or arrow application (f -< arg)
        (XCmdArrApp id)  -- type of the arrow terms f,
                         -- of the form a t t', where arg :: t
        (LHsTerm id)     -- arrow term, f
        (LHsTerm id)     -- input term, arg
        HsArrAppType     -- higher-order (-<<) or first-order (-<)
        Bool             -- True => right-to-left (f -< arg)
                         -- False => left-to-right (arg >- f)
  ...
  | HsCmdIf     (XCmdIf id)
                (Maybe (SyntaxTerm id))         -- cond function
                (LHsTerm id)                    -- predicate
                (LHsCmd id)                     -- then part
                (LHsCmd id)                     -- else part
    -- ^ - 'ApiAnnotation.AnnKeywordId' : 'ApiAnnotation.AnnIf',
    --       'ApiAnnotation.AnnSemi',
  ...
  | HsCmdDo     (XCmdDo id)                     -- Type of the whole term
                (Located [CmdLStmt id])
    -- ^ - 'ApiAnnotation.AnnKeywordId' : 'ApiAnnotation.AnnDo',
    --             'ApiAnnotation.AnnOpen', 'ApiAnnotation.AnnSemi',
    --             'ApiAnnotation.AnnVbar',
...
ppr_cmd (HsCmdCase _ term matches)
  = sep [ sep [text "case", nest 4 (ppr term), ptext (sLit "of")],
          nest 2 (pprMatches matches) ]
...
ppr_cmd (HsCmdDo _ (L _ stmts))  = pprDo ArrowTerm stmts
...
type HsRecordBinds p = HsRecFields p (LHsTerm p)
...
-- | Term Located Statement
type TermLStmt  id = LStmt id (LHsTerm id)

-- | Term Statement
type TermStmt   id = Stmt  id (LHsTerm id)

-- | Guard Located Statement
type GuardLStmt id = LStmt id (LHsTerm id)

-- | Guard Statement
type GuardStmt  id = Stmt  id (LHsTerm id)

-- | Ghci Located Statement
type GhciLStmt  id = LStmt id (LHsTerm id)

-- | Ghci Statement
type GhciStmt   id = Stmt  id (LHsTerm id)

-- The SyntaxTerms in here are used *only* for do-notation and monad
...
data StmtLR idL idR body -- body should always be (LHs**** idR)
  = LastStmt  -- Always the last Stmt in ListComp, MonadComp,
              -- and (after the renamer) DoTerm, MDoTerm
              -- Not used for GhciStmtCtxt, PatGuard, which scope over other stuff
          (XLastStmt idL idR body)
          body
          Bool               -- True <=> return was stripped by ApplicativeDo
          (SyntaxTerm idR)   -- The return operator, used only for
                             -- MonadComp For ListComp we
                             -- use the baked-in 'return' For DoTerm,
                             -- MDoTerm, we don't apply a 'return' at
  ...
  | BindStmt (XBindStmt idL idR body) -- Post typechecking,
             ...
             (LPat idL)
             body
             (SyntaxTerm idR) -- The (>>=) operator; see Note [The type of bind in Stmts]
             (SyntaxTerm idR) -- The fail operator
             -- The fail operator is noSyntaxTerm
             -- if the pattern match can't fail

  -- | 'ApplicativeStmt' represents an applicative term built with
  -- <$> and <*>.  It is generated by the renamer, and is desugared into the
  -- appropriate applicative term by the desugarer, but it is intended
  -- to be invisible in error messages.
  --
  -- For full details, see Note [ApplicativeDo] in RnTerm
  --
  | ApplicativeStmt
             ...
             [ ( SyntaxTerm idR
               , ApplicativeArg idL) ]
                      -- [(<$>, e1), (<*>, e2), ..., (<*>, en)]
             (Maybe (SyntaxTerm idR))  -- 'join', if necessary

  | BodyStmt (XBodyStmt idL idR body) -- Post typecheck, element type
                                      -- of the RHS (used for arrows)
             body              -- See Note [BodyStmt]
             (SyntaxTerm idR)  -- The (>>) operator
             (SyntaxTerm idR)  -- The `guard` operator; used only in MonadComp
  ...
  | ParStmt  (XParStmt idL idR body)    -- Post typecheck,
             ...
             (HsTerm idR)               -- Polymorphic `mzip` for monad comprehensions
             (SyntaxTerm idR)           -- The `>>=` operator
                                        -- See notes [Monad Comprehensions]
  ...
  | TransStmt {
      trS_ext   :: XTransStmt idL idR body, -- Post typecheck,
                                            -- R in (>>=) :: Q -> (R -> S) -> T
      trS_form  :: TransForm,
      trS_stmts :: [TermLStmt idL],   -- Stmts to the *left* of the 'group'
                                      -- which generates the tuples to be grouped

      trS_bndrs :: [(IdP idR, IdP idR)], -- See Note [TransStmt binder map]

      trS_using :: LHsTerm idR,
      trS_by :: Maybe (LHsTerm idR),  -- "by e" (optional)
        -- Invariant: if trS_form = GroupBy, then grp_by = Just e

      trS_ret :: SyntaxTerm idR,      -- The monomorphic 'return' function for
                                      -- the inner monad comprehensions
      trS_bind :: SyntaxTerm idR,     -- The '(>>=)' operator
      trS_fmap :: HsTerm idR          -- The polymorphic 'fmap' function for desugaring
                                      -- Only for 'group' forms
                                      -- Just a simple HsTerm, because it's
  ...
  | RecStmt
     ...
     , recS_rec_ids :: [IdP idR]
        ...
        -- Rebindable syntax
     , recS_bind_fn :: SyntaxTerm idR -- The bind function
     , recS_ret_fn  :: SyntaxTerm idR -- The return function
     , recS_mfix_fn :: SyntaxTerm idR -- The mfix function
      }
...
data RecStmtTc =
  RecStmtTc
     ...
     , recS_later_rets :: [PostTcTerm] -- (only used in the arrow version)
     , recS_rec_rets :: [PostTcTerm] -- These terms correspond 1-to-1
                                  -- with recS_later_ids and recS_rec_ids,
                                  -- and are the terms that should be
...
data ParStmtBlock idL idR
  = ParStmtBlock
        ...
        (SyntaxTerm idR)   -- The return operator
...
data ApplicativeArg idL
  ...
  | ApplicativeArgMany     -- do { stmts; return vars }
      ...
      [TermLStmt idL]      -- stmts
      (HsTerm idL)         -- return (v1,..,vn), or just (v1,..,vn)
      (LPat idL)           -- (v1,...,vn)
...
term is used to lift the body of the monad comprehension:
...
instance (Outputable (StmtLR idL idL (LHsTerm idL)),
...
pprStmt (LastStmt _ term ret_stripped _)
  = whenPprDebug (text "[last]") <+>
       (if ret_stripped then text "return" else empty) <+>
       ppr term
pprStmt (BindStmt _ pat term _ _) = hsep [ppr pat, larrow, ppr term]
pprStmt (LetStmt _ (L _ binds))   = hsep [text "let", pprBinds binds]
pprStmt (BodyStmt _ term _ _)     = ppr term
...
pprStmt (ApplicativeStmt _ args mb_join)
  ...
  -- of statements.
   ...
   -- type.
   flattenStmt :: TermLStmt (GhcPass idL) -> [SDoc]
   flattenStmt (L _ (ApplicativeStmt _ args _)) = concatMap flattenArg args
   flattenStmt stmt = [ppr stmt]

   flattenArg :: forall a . (a, ApplicativeArg (GhcPass idL)) -> [SDoc]
   flattenArg (_, ApplicativeArgOne _ pat term isBody)
     | isBody =  -- See Note [Applicative BodyStmt]
     [ppr (BodyStmt (panic "pprStmt") term noSyntaxTerm noSyntaxTerm
             :: TermStmt (GhcPass idL))]
     | otherwise =
     [ppr (BindStmt (panic "pprStmt") pat term noSyntaxTerm noSyntaxTerm
             :: TermStmt (GhcPass idL))]
   ...
   pp_debug =
     let
         ap_term = sep (punctuate (text " |") (map pp_arg args))
     in
       if isNothing mb_join
          then ap_term
          else text "join" <+> parens ap_term

   pp_arg :: (a, ApplicativeArg (GhcPass idL)) -> SDoc
   pp_arg (_, ApplicativeArgOne _ pat term isBody)
     | isBody =  -- See Note [Applicative BodyStmt]
     ppr (BodyStmt (panic "pprStmt") term noSyntaxTerm noSyntaxTerm
            :: TermStmt (GhcPass idL))
     | otherwise =
   ...
   pp_arg (_, ApplicativeArgMany _ stmts return pat) =
     ppr pat <+>
     text "<-" <+>
     ppr (HsDo (panic "pprStmt") DoTerm (noLoc
               (stmts ++
                   [noLoc (LastStmt noExt (noLoc return) False noSyntaxTerm)])))
   pp_arg (_, XApplicativeArg x) = ppr x

pprStmt (XStmtLR x) = ppr x

pprTransformStmt :: (OutputableBndrId (GhcPass p))
                 => [IdP (GhcPass p)] -> LHsTerm (GhcPass p)
                 -> Maybe (LHsTerm (GhcPass p)) -> SDoc
...
pprDo DoTerm        stmts = text "do"  <+> ppr_do_stmts stmts
pprDo GhciStmtCtxt  stmts = text "do"  <+> ppr_do_stmts stmts
pprDo ArrowTerm     stmts = text "do"  <+> ppr_do_stmts stmts
pprDo MDoTerm       stmts = text "mdo" <+> ppr_do_stmts stmts
...
data HsSplice id
   = HsTypedSplice       --  $$z  or $$(f 4)
        ...
        (LHsTerm id)     -- See Note [Pending Splices]
...
instance Data ThModFinalizers where
  ...
  dataTypeOf a  = mkDataType "HsTerm.ThModFinalizers" [toConstr a]

-- | Haskell Spliced Thing
--
-- Values that can result from running a splice.
data HsSplicedThing id
    = HsSplicedTerm (HsTerm id) -- ^ Haskell Spliced Term
...
data PendingRnSplice
  = PendingRnSplice UntypedSpliceFlavour SplicePointName (LHsTerm GhcRn)
...
data PendingTcSplice
  ...
  = PendingTcSplice SplicePointName (LHsTerm GhcTc)
...
* The PendingRnExpSplice gives the splice that splice-point name maps to;
  and the typechecker can now conveniently find these sub-terms
 ...
 * Pending term splices (UntypedExpSplice), e.g.,
       [|$(f x) + 2|]

   UntypedExpSplice is also used for
     * quasi-quotes, where the pending term expands to
          $(quoter "...blah...")
       (see RnSplice.makePending, HsQuasiQuote case)

     * cross-stage lifting, where the pending term expands to
          $(lift x)
...
checker:
  ...
  * Pending *typed* term splices, (PendingTcSplice), e.g.,
        [||1 + $$(f 2)||]
...
sense, although I hate to add another constructor to HsTerm.
-}

instance (p ~ GhcPass pass, OutputableBndrId p)
       => Outputable (HsSplicedThing p) where
  ppr (HsSplicedTerm e) = ppr_term e
...
pprPendingSplice :: (OutputableBndrId (GhcPass p))
                 => SplicePointName -> LHsTerm (GhcPass p) -> SDoc
...
ppr_splice :: (OutputableBndrId (GhcPass p))
           => SDoc -> (IdP (GhcPass p)) -> LHsTerm (GhcPass p) -> SDoc -> SDoc
ppr_splice herald n e trail
    = herald <> whenPprDebug (brackets (ppr n)) <> ppr e <> trail

-- | Haskell Bracket
data HsBracket p
  = ExpBr  (XExpBr p)   (LHsTerm p)    -- [|  term  |]
  ...
  | TExpBr (XTExpBr p) (LHsTerm p)    -- [||  term  ||]
...
data HsMatchContext id -- Not an extensible tag
  = FunRhs { mc_fun        :: Located id    -- ^ function binder of @f@
           ...
           , mc_strictness :: SrcStrictness -- ^ was @f@ banged?
                                ...
                                -- function binding
  | LambdaTerm                  -- ^Patterns of a lambda
  | CaseAlt                     -- ^Patterns and guards on a case alternative
  | IfAlt                       -- ^Guards of a multi-way if alternative
  | ProcTerm                    -- ^Patterns of a proc
  | PatBindRhs                  -- ^A pattern binding  eg [y] <- e = e
  | PatBindGuards               -- ^Guards of pattern bindings, e.g.,
                                --    (Just b) | Just _ <- x = e
                                --             | otherwise   = e'

  | RecUpd                      -- ^Record update [used only in DsTerm to
...
instance OutputableBndr id => Outputable (HsMatchContext id) where
  ...
  ppr LambdaTerm            = text "LambdaTerm"
  ppr CaseAlt               = text "CaseAlt"
  ppr IfAlt                 = text "IfAlt"
  ppr ProcTerm              = text "ProcTerm"
...
data HsStmtContext id
  ...
  | DoTerm                           -- ^do { ... }
  | MDoTerm                          -- ^mdo { ... }  ie recursive do-term
  | ArrowTerm                        -- ^do-notation in an arrow-command context
...
isListCompTerm :: HsStmtContext id -> Bool
-- Uses syntax [ e | quals ]
isListCompTerm ListComp          = True
isListCompTerm MonadComp         = True
isListCompTerm (ParStmtCtxt c)   = isListCompTerm c
isListCompTerm (TransStmtCtxt c) = isListCompTerm c
isListCompTerm _ = False

-- | Should pattern match failure in a 'HsStmtContext' be desugared using
-- 'MonadFail'?
isMonadFailStmtContext :: HsStmtContext id -> Bool
isMonadFailStmtContext MonadComp            = True
isMonadFailStmtContext DoTerm               = True
isMonadFailStmtContext MDoTerm              = True
...
isMonadFailStmtContext _ = False -- ListComp, PatGuard, ArrowTerm

matchSeparator :: HsMatchContext id -> SDoc
matchSeparator (FunRhs {})   = text "="
matchSeparator CaseAlt       = text "->"
matchSeparator IfAlt         = text "->"
matchSeparator LambdaTerm    = text "->"
matchSeparator ProcTerm      = text "->"
...
pprMatchContextNoun LambdaTerm      = text "lambda abstraction"
pprMatchContextNoun ProcTerm        = text "arrow abstraction"
...
pprStmtContext DoTerm          = text "'do' block"
pprStmtContext MDoTerm         = text "'mdo' block"
pprStmtContext ArrowTerm       = text "'do' block in an arrow command"
...
matchContextErrString LambdaTerm                 = text "lambda"
matchContextErrString ProcTerm                   = text "proc"
...
matchContextErrString (StmtCtxt DoTerm)            = text "'do' block"
matchContextErrString (StmtCtxt ArrowTerm)         = text "'do' block"
matchContextErrString (StmtCtxt MDoTerm)           = text "'mdo' block"
...
pprStmtInCtxt ctxt (LastStmt _ e _ _)
  | isListCompTerm ctxt      -- For [ e | .. ], do not mutter about "stmts"
  = hang (text "In the term:") 2 (ppr e)
